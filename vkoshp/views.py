...
def index(request):
	...
	return render(request, 'index.html', context={'codeforces_link':codeforces_link,'contest':contest,'teams':teams_sorted})

from django.shortcuts import get_object_or_404
from django.urls import reverse

from .forms import AddParticipantHandleForm, UrlForm
...
def add_handle(request,participant_id=None):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			handleinvoice = HandleInvoice(participant = participant, handle = codeforces_handle)
			handleinvoice.save()
...
from .parser import parse_contest, process_handles, process_handle, get_rating
...
def process_invoices(request):
	try: 
		...
		return render(request, 'invoice.html', context={'profile':process_handle(handleinvoice.handle),'handleinvoice':handleinvoice})
	except IndexError:
		return HttpResponseRedirect(reverse('success') )
